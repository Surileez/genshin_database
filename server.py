    ...
    if request.method == 'GET':
        ...
        if submit == 'search':
            if not (inuid.isdigit() or inuid == ''):
                context = dict(data=[])
                return render_template('users.html', data=[], wrongu='must input integer')
            if not (inulevel.isdigit() or inulevel == ''):
                context = dict(data=[])
                return render_template('users.html', data=[], wrongl='must input integer')
            if not (inday.isdigit() or inday == ''):
                context = dict(data=[])
                return render_template('users.html', data=[], wrongd='must input integer')
            if not (inachievements.isdigit() or inachievements == ''):
                context = dict(data=[])
                return render_template('users.html', data=[], wronga='must input integer')
            else:
                ...
                context = dict(data=weapons)
                cursor.close()
                return render_template('users.html', data=dweapons)
...
def owning():
    ...
    if request.method == 'GET':
        cursor = g.conn.execute(
            "SELECT O.uid,uname,cname,elements,character_rarity,clevel,friendship,constellation from Owning O, Users U, Characters C where O.uid=U.uid and O.cid=C.cid limit 5")
        characters = []
        for result in cursor:
            characters.append(result)  # can also be accessed using result
        context = dict(data=characters)
        ...
        if submit == 'search':
            ...
            if not (incon.isdigit() or incon == ''):
                ...
                cols = ['O.uid', 'uname', 'cname', 'elements', 'character_rarity', 'clevel', 'friendship',
                        'constellation']
                dic = {}
                i = 1
                for col in cols:
...
def materials():
    ...
    if request.method == 'GET':
        ...
        loc_material = []
        for result in cursor:
            loc_material.append(result)
        context_l = dict(data_l=loc_material)
        cursor.close()
        ...
        t_material = []
        for result in cursor:
            t_material.append(result)
        context_t = dict(data_t=t_material)
        cursor.close()
        return render_template("materials.html", data_l=loc_material, data_t=t_material)
    else:
        getcol = []
        ...
        if submit == 'search':
            ...
            if (intype != []) and ('Talent_level_up_materials' not in intype):
                ...
                Talent = []
            return render_template('materials.html', data_l=Loc, data_t=Talent)
...
def special():
    ...
    if request.method == 'GET':
        ...
        if order_target == None:
            c1 = dict(data_u=[])
            c2 = dict(data_c=[])
            return render_template('special.html', data_u=[], data_c=[], wrong='Please choose one order target!')
        ...
        if submit == 'search':
            if orderu == None and orderb == None:
                c1 = dict(data_u=[])
                c2 = dict(data_c=[])
                return render_template('special.html', data_u=[], data_c=[], wrong='Please choose one order type!')
            if (('Both' in order_target) or ('Character' in order_target)) and orderb == None:
                c1 = dict(data_u=[])
                c2 = dict(data_c=[])
                return render_template('special.html', data_u=[], data_c=[], wrong='Please match order_target and order_type!')
